...
def calculate_school_kpis(students_df, academic_df, financial_df, employees_df):
    ...
    if not academic_df.empty:
        ...
        if not current_grades.empty:
            ...
            kpis['avg_grade'] = 0.0
            kpis['approval_rate'] = 0.0
            kpis['students_at_risk'] = 0
    else:
        kpis['avg_grade'] = 0.0
        kpis['approval_rate'] = 0.0
        kpis['students_at_risk'] = 0
    ...
    current_financial = financial_df[financial_df['month'] == current_month] if not financial_df.empty else pd.DataFrame()
    
    if not current_financial.empty:
        ...
        kpis['monthly_revenue'] = 0.0
        kpis['defaulting_students'] = 0
    
    # KPIs Operacionais
    active_students = len(students_df[students_df['status'] == 'Ativo']) if not students_df.empty else 0
    kpis['total_students'] = active_students
    
    pedagogical_staff = len(employees_df[employees_df['department'] == 'Pedagógico']) if not employees_df.empty else 1
    kpis['student_teacher_ratio'] = active_students / pedagogical_staff if pedagogical_staff > 0 else 0
    ...
    if not employees_df.empty:
        ...
        kpis['employee_absences'] = 0.0
        kpis['teacher_count'] = 0
...
def create_grade_evolution_chart(academic_df):
    ...
    if academic_df.empty:
        ...
        fig.update_layout(
            ...
            annotations=[dict(text="Nenhum dado acadêmico disponível", 
    ...
    if bimester_grades.empty:
        fig = go.Figure()
        fig.update_layout(
            title="Nenhum dado disponível",
            annotations=[dict(text="Nenhum dado por bimestre disponível", 
                            xref="paper", yref="paper", x=0.5, y=0.5,
                            showarrow=False, font=dict(size=16))]
        )
        return fig
...
def create_performance_by_segment_chart(academic_df):
    ...
    if segment_performance.empty:
        ...
        fig.update_layout(
            title="Nenhum dado disponível",
            annotations=[dict(text="Nenhum dado por segmento disponível", 
                            xref="paper", yref="paper", x=0.5, y=0.5,
                            showarrow=False, font=dict(size=16))]
        )
        return fig
...
def render_academic_dashboard(students_df, academic_df, kpis):
    ...
    with col2:
        ...
        # Versão ultra-robusta do histograma
        try:
            if not academic_df.empty and len(academic_df) > 0:
                # Filtrar dados válidos
                grades_series = academic_df['grade']
                valid_grades = grades_series.dropna()
                
                if len(valid_grades) > 0:
                    # Converter para lista para garantir compatibilidade
                    grades_list = valid_grades.tolist()
                    
                    # Criar histograma usando go.Figure para máximo controle
                    ...
                    fig_histogram.update_layout(
                        title={'text': '<b>Distribuição das Notas</b>', 'font': {'size': 20, 'family': 'Inter'}, 'x': 0.02},
                        xaxis_title="Nota",
                    ...
                    st.info("📊 Não há notas válidas para exibir no histograma")
            else:
                st.info("📊 Nenhum dado acadêmico disponível para os filtros selecionados")
                
        except Exception as e:
            # Fallback em caso de qualquer erro
            st.warning("⚠️ Erro ao gerar gráfico de distribuição")
            
            # Mostrar informações básicas como alternativa
            if not academic_df.empty:
                st.write("**Estatísticas Básicas das Notas:**")
                try:
                    grade_stats = academic_df['grade'].describe()
                    st.write(f"- Média: {grade_stats['mean']:.1f}")
                    st.write(f"- Mínima: {grade_stats['min']:.1f}")
                    st.write(f"- Máxima: {grade_stats['max']:.1f}")
                except:
                    st.write("Não foi possível calcular estatísticas")
    ...
    if not academic_df.empty:
        ...
        if not class_performance.empty:
            class_performance = class_performance.sort_values('grade', ascending=False)
            ...
            with col1:
                ...
                if not top_classes.empty:
                    ...
                    st.info("📊 Nenhuma turma disponível nos filtros selecionados")
            ...
            st.info("📊 Nenhum dado de performance por turma disponível")
    else:
        st.info("📊 Nenhum dado acadêmico disponível para análise de turmas")
